...
const RentalForm = ({ initialValues, onSubmit, onCancel, submitLabel = 'Guardar Alquiler', isSubmitting = false }) => {
        ...
        if (initialValues) {
            ...
            if (initialValues.clientData) {
                try {
                    ...
                    console.error("Error parsing clientData in RentalForm:", e);
                    // Fallback
    ...
    const handleSubmit = async () => {
        ...
        if (isInstallment && installmentAmountNum > totalAmount) {
            showAlert("error", "Error", "El abono no puede ser mayor al valor total del alquiler");
            return;
        }
            ...
            >
                ...
                >
                    ...
                    <View style={styles.sectionContainer}>
                        ...
                        <Text style={styles.label}>Descripción del Alquiler (Opcional)</Text>
                        <TextInput
                            style={styles.input}
                            placeholder="Ej: Equipo Fotográfico..."
                    ...
                    {/* SECCIÓN 4: INVENTARIO ALQUILADO */}
                    <View style={styles.sectionContainer}>
                    ...
                    <View style={styles.sectionContainer}>
                        ...
                        <View style={[styles.totalContainer, { marginBottom: 0 }]}>
                            <View>
                                <Text style={styles.totalLabel}>{isInstallment ? 'Total Alquiler' : 'Total Alquiler'}</Text>
                                <Text style={styles.statusBadge}>Estado: Activo</Text>
                            </View>
                            <Text style={styles.totalAmount}>{formatCurrency(totalAmount)}</Text>
                        </View>

                        {isInstallment && (
                            <View style={[styles.totalContainer, { marginTop: SPACING.sm, backgroundColor: '#8B5CF6' + '20', borderColor: '#8B5CF6' }]}>
